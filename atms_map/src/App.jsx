    ...
    useEffect(() => {
        if (token) {
            fetch('/accounts/me', { headers: { 'Authorization': `Bearer ${token}` } })
                ...
                .then(setAccounts).catch(() => { setToken(null); localStorage.removeItem('map_token'); })
        }
    }, [token])
    ...
    return (
        <div className="h-screen w-full flex flex-col bg-slate-900 text-slate-100 font-sans">
            <header className="h-16 bg-slate-950 border-b border-indigo-900/30 flex items-center justify-between px-6 shadow-lg z-[2000] relative">
                <h1 className="text-xl font-bold tracking-wider text-indigo-400">POINT<span className="text-white">MAP</span></h1>
                <div className="flex gap-3">
                    ...
                    {token ? (
                        <div className="flex items-center gap-4">
                            ...
                            <button onClick={() => {setToken(null); localStorage.removeItem('map_token')}} className="px-4 py-1.5 text-sm bg-red-500/10 text-red-400 border border-red-500/20 rounded hover:bg-red-500/20 transition">Logout</button>
                        </div>
                    ) : (
