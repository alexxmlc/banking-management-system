...
function MapInputHandler({ active, onSelect }) {
  useMapEvents({
    click(e) {
      if (active) onSelect([e.latlng.lat, e.latlng.lng]);
    },
  });
  return null;
}
  ...
  useEffect(() => {
    ...
    const routingControl = L.Routing.control({
      waypoints: [
        ...
        L.latLng(targetAtm.latitude, targetAtm.longitude),
      ],
...
function MapController({ selectedAtm, userPos }) {
  ...
  useEffect(() => {
    if (selectedAtm && !userPos) {
      map.flyTo([selectedAtm.latitude, selectedAtm.longitude], 16, {
        duration: 1.2,
      });
    }
      ...
      {!token ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          >
            {accounts.map((acc) => (
              <option key={acc.id} value={acc.iban}>
                {acc.currency} •••• {acc.iban.slice(-4)} (Bal: {acc.balance})
              </option>
            ))}
          </select>
          ...
          <input
            ...
            placeholder="Amount"
            required
            className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm outline-none focus:border-indigo-500"
            value={amount}
...
function FindUs() {
  ...
  const performTransaction = async (type, iban, amt) => {
    ...
    const accRes = await fetch("/accounts/me", {
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    <div className="h-screen w-full flex flex-col bg-slate-900 text-slate-100 font-sans">
      <header className="h-16 bg-slate-950 border-b border-indigo-900/30 flex items-center justify-between px-6 shadow-lg z-[2000] relative">
        <h1 className="text-xl font-bold tracking-wider text-indigo-400">
          POINT<span className="text-white">MAP</span>
        </h1>

        <div className="flex gap-3">
          <button
            onClick={handleLocateMe}
            className="px-4 py-1.5 text-sm bg-slate-800 text-indigo-300 border border-indigo-500/30 rounded hover:bg-slate-700 transition flex items-center gap-2"
          >
            📍 Locate Me
          </button>
          ...
          {token ? (
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="px-4 py-1.5 text-sm bg-red-500/10 text-red-400 border border-red-500/20 rounded hover:bg-red-500/20 transition"
              >
                Logout
              </button>
            </div>
            ...
            <button
              ...
              className="px-5 py-2 text-sm font-semibold bg-indigo-600 hover:bg-indigo-500 rounded text-white shadow-lg transition"
            >
              Login
            </button>
          )}
      ...
      <div className="flex-1 relative z-0">
            ...
            {atms
              ...
              .map((atm) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-3 rounded-lg text-sm transition flex items-center justify-between group ${
        ...
        {/* MAP */}
        <MapContainer
          center={[46.7712, 23.6236]} // Cluj-Napoca default
          zoom={13}
          style={{
            height: "100%",
            width: "100%",
            cursor: isMovingUser ? "crosshair" : "grab",
          }}
        >
          <TileLayer
              ...
              <Popup>
                <div className="text-slate-900 min-w-[150px]">
                  ...
                  <p className="text-xs text-slate-600">
                    Lat: {atm.latitude}, Lon: {atm.longitude}
                  </p>
                </div>
              </Popup>
            </Marker>
            ...
            <div className="bg-slate-900 border border-indigo-500/30 p-6 rounded-2xl w-full max-w-sm shadow-2xl">
              ...
              <form onSubmit={login} className="space-y-3">
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 outline-none focus:border-indigo-500"
                  value={username}
                ...
                <div className="flex gap-2 mt-4">
                  ...
                  <button
                    type="submit"
                    className="flex-1 py-2 bg-indigo-600 rounded text-white font-semibold hover:bg-indigo-500"
                  >
                    Login
                  </button>
                </div>
